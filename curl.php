...
class Curl extends Service
{
  ...
  public function attachFile($name, $mimeType, $filePath)
  {
    $this->files[] = (object) [
      'name' => $name,
      'mimeType' => $mimeType,
    ...
    $this->payload[$name] = new CURLFile(realpath($filePath), $mimeType);
    return $this;
  }
  ...
  public function request($httpVerb, $url)
  {
    ...
    switch ($httpVerb) {
      ...
      case 'PATCH':
        ...
        break;
      case 'DELETE':
        if (!empty($this->files)) throw new Exception("You cannot send files via DELETE method.");
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "DELETE");
        curl_setopt($ch, CURLOPT_POSTFIELDS,  $this->payload);
        break;
      case 'GET':
        if (!empty($this->files)) throw new Exception("You cannot send files via GET method.");
    ...
    if (curl_errno($ch))
      throw new Exception("cURL Error nº " . curl_errno($ch) . ": '" . curl_error($ch) . "'");
